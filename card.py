...
class CommonJinnangCard(JinnangCard):
    def __init__(self, name, color, point):
        ...
        if self.name == "顺手牵羊":
            ...
            self.name == "万箭齐发" or self.name == "南蛮入侵"
        ):  # 万箭齐发和南蛮入侵的目标为所有其他角色
            self.target = ["all other players"]
        elif (
            self.name == "五谷丰登" or self.name == "桃园结义"
        ):  # 五谷丰登和桃园结义的目标为所有角色
...
class GetCardHeap:  # 摸牌堆
    ...
    def get_card(self, num, left_card_heap):  # 摸n张牌
        ...
        if len(self.card_list) >= num:
            ...
            len(left_card_heap.card_list) + len(self.card_list)
            >= num
            > len(self.card_list)
            >= 0
        ):
            ...
            return_card = return_card + self.get_card(
                num - len(return_card), left_card_heap
            )
