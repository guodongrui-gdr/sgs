...
def Game_Process(player: player):
    ...
    for time in game_process_tmp:
        ...
        if time == 'pandin':
            ...
            for card in get_card_heap.get_card(2, left_card_heap):
                    ...
                    while 1:
                        ...
                        if card.name == '铁索连环':
                            ...
                            if not player_input:
                                ...
                                player.HandCards_area.append(get_card_heap.get_card(1, left_card_heap)[0])
                                break
...
def Pandin_Process(player):
    ...
    for time in pandin_process:
        # check_skill(time) 检查是否有武将技能发动
        if time == 'when_pandin':
            res_card = get_card_heap.get_card(1, left_card_heap)
...
def Use_Card_process(card: Card.card,
    ...
    if '杀' in card.name:
        ...
        if player.current_HP == player.max_HP:  # 若当前玩家体力值等于体力上限,则无法指定自己为目标
            return
    elif card.name == '闪' and target_card is None:
        return
    # 选择目标
    # check_skill()
    target = []
    ...
    if card.name == '五谷丰登':
        for i in range(len(player_list)):
            tmp_card.append(get_card_heap.get_card(1, left_card_heap)[0])
        print([[card.name, card.color, card.point] for card in tmp_card])
...
def Use_Clear_Process(player: player,
    ...
    for i in range(len(target)):
        ...
        if card.name == '桃园结义' and target[i].current_HP == target[i].max_HP:
            continue
        # 生效前
        if type(card) == Card.jinnang_card:
            for j in player_list:
                if '无懈可击' not in [wuxie.name for wuxie in j.HandCards_area]:
                    continue
                wuxie = eval(input('{}号位是否使用无懈可击'.format(j.idx)))
                if not wuxie:
                    continue
        ...
        if '杀' in card.name:
            ...
            sha_idx: card = Card.card(None, None, None)
            while 1:
        ...
        if '杀' in card.name:
            ...
            for get_card in get_card_heap.get_card(2, left_card_heap):
                player.HandCards_area.append(get_card)
        elif card.name == '五谷丰登':
            idx = eval(input('{}号位请选择获得一张牌:'.format(target[i].idx))) - 1
            target[i].HandCards_area.append(tmp_card[idx])
            del tmp_card[idx]
            print([[card.name, card.color, card.point] for card in tmp_card])
        elif card.name == '铁索连环':
            target[i].hengzhi = not target[i].hengzhi
        elif card.name == '火攻':
            show_card: Card.card = target[i].HandCards_area[eval(input('{}号位请展示一张手牌'.format(target[i].idx))) - 1]
            print(show_card.name, show_card.color, show_card.point)
            left_card: int = eval(input('{}号位可以弃置一张与之花色相同的牌,对其造成1点火焰伤害:'))
            if not left_card:
                continue
            ...
            while 1:
                ...
                print('目标装备区有武器牌:{},防具牌:{}, 进攻坐骑:{}, 防御坐骑:{},宝物:{}'.format(target[i].equipment_area['武器'].name,
                ...
                player_input = input('请选择一张牌弃置(i表示弃置第i张手牌,w表示弃置武器牌,a表示弃置防具牌,'
                                     '-1表示弃置进攻坐骑,+1表示弃置防御坐骑,t表示弃置宝物牌,l表示弃置乐不思蜀,'
                                     'b表示弃置兵粮寸断,s表示弃置闪电):')
                try:
                    player_input = eval(player_input)
                except NameError:
                    if player_input == 'w' and target[i].equipment_area['武器'].name is not None:
                        left_card_heap.card_list.append(target[i].equipment_area['武器'])
                        ...
                        left_card_heap.card_list.append(target[i].equipment_area['防具'])
                        ...
                        left_card_heap.card_list.append(target[i].equipment_area['进攻坐骑'])
                        ...
                        left_card_heap.card_list.append(target[i].equipment_area['防御坐骑'])
                        ...
                        left_card_heap.card_list.append(target[i].equipment_area['宝物'])
                        ...
                        left_card_heap.card_list.append(
                            [card for card in target[i].pandin_area if card.name == '兵粮寸断'][0])
                        target[i].pandin_area.remove([card for card in target[i].pandin_area if card.name == '兵粮寸断'][0])
                        break
                        ...
                        target[i].pandin_area.remove([card for card in target[i].pandin_area if card.name == '乐不思蜀'][0])
                        break
                ...
                try:
                    left_card = target[i].HandCards_area[player_input]
                    break
            ...
            print('被弃置的牌为:{}'.format(left_card_heap.card_list[0].name))
        elif card.name == '顺手牵羊':
            ...
            while 1:
                ...
                player_input = input('请选择一张牌获得(i表示获得第i张手牌,w表示获得武器牌,a表示获得防具牌,'
                                     '-1表示获得进攻坐骑,+1表示获得防御坐骑,t表示获得宝物牌,l表示获得乐不思蜀,'
                                     'b表示获得兵粮寸断,s表示获得闪电):')
                try:
                    player_input = eval(player_input)
                except NameError:
                    if player_input == 'w' and target[i].equipment_area['武器'].name is not None:
                        ...
                        player.HandCards_area.append([card for card in target[i].pandin_area if card.name == '兵粮寸断'][0])
                        target[i].pandin_area.remove([card for card in target[i].pandin_area if card.name == '兵粮寸断'][0])
                        break
                    elif player_input == 'l' and '乐不思蜀' in [card.name for card in target[i].pandin_area]:
                        player.HandCards_area.append([card for card in target[i].pandin_area if card.name == '乐不思蜀'][0])
                        target[i].pandin_area.remove([card for card in target[i].pandin_area if card.name == '乐不思蜀'][0])
                        break
                    elif player_input == 's' and '闪电' in [card.name for card in target[i].pandin_area]:
